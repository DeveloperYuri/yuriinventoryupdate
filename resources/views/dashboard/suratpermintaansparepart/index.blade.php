    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex justify-content-between align-items-center">
            @if (Auth::user()->is_role == 2 || Auth::user()->is_role == 0)
                <a href="{{ route('suratpermintaansparepart.create') }}" class="btn btn-primary" dusk="addsparepart">Buat Surat
                    Permintaan Spare Part</a>
            @endif
                ...
                <div class="row g-2 align-items-center">
                    <div class="col">
                        <input id="searchingtitle" type="text" class="form-control" value="{{ Request()->name }}"
                            placeholder="Cari Surat Permintaan Spare Part" name="name">
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5 class="card-title mb-0">Daftar Surat Permintaan Spare Part</h5>
                                ...
                                <table class="table table-hover align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($getRecord as $index => $sp)
                                            <tr onclick="window.location='{{ route('suratpermintaansparepart.show', $sp->id) }}'"
                                                style="cursor:pointer;">
                                                <td class="text-center">
                                                    {{ !empty($sp->no_surat_permintaan) ? $sp->no_surat_permintaan : '000-000-000' }}
                                                </td>
                                                ...
                                                <td class="text-nowrap">
                                                    @if ($sp->status == 'draft')
                                                        <a href="{{ route('suratpermintaansparepart.edit', $sp->id) }}"
                                                            class="btn btn-sm btn-primary">Edit</a>

                                                        <form action="{{ route('suratpermintaansparepart.delete', $sp->id) }}"
                                                        ...
                                                        <form action="{{ route('suratpermintaansparepart.submit', $sp->id) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit"
                                                    ...
                                                    @elseif ($sp->status == 'onprogress' && (auth()->user()->is_role == 1 || auth()->user()->is_role == 2))
                                                        <form action="{{ route('suratpermintaansparepart.approve', $sp->id) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit"
                                                        ...
                                                        <form action="{{ route('suratpermintaansparepart.reject', $sp->id) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit"
